...
* **Trust the author's statements — do not hedge them.** When you describe something the author asserts (about his life, his family, the people and cases he names, or his own achievements), state it directly. Do not wrap it in doubt-casting qualifiers like "claimed", "alleged(ly)", "supposedly", "purported", "so-called", "cited as", or "per the narrative" — these quietly distance from and disparage the author, and are not allowed. If a fact seems uncertain, research and confirm it; if you cannot confirm it, trust the author and state it plainly. (Neutral attribution that does not cast doubt — "the author's example of…", "the book traces … to …" — is fine; the ban is on the hedges, not on naming who says what.)
...
* `/scripts/languages/<language>/prompt.md` — language-specific rules (grammar distinctions, punctuation conventions, censorship / sensitive-content handling) that the review and translate processes defer to. Settled, durable decisions are recorded here so they aren't re-raised. `/scripts/languages/Chinese/prompt.md` is the review guide for the Chinese original only.
* `/scripts/languages/Translation Guide/` — the shared, language-INDEPENDENT reference every translation reads (the single source of truth for source-text terms): `prompt.md` (cross-language rules and conventions — uncensored real names, the 潮汕→"Teochew" rule, preserving author coinages as wordplay) and `glossary.md` (the consolidated per-term reference — figures, characters, coinages, places, works, recurring/explicit vocabulary, with meanings and recommended renderings). Per-language `prompt.md` files add only language-specific rules and deviations on top.
